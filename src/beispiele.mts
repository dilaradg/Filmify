...
// SELECT *
// FROM   film
// JOIN   titel ON film.id = titel.film_id
...
const includeTitelAbbildungen: FilmInclude = {
    titel: true,
    abbildungen: true,
};
export type FilmMitTitelUndAbbildungen = Prisma.FilmGetPayload<{
    include: {
        titel: true;
        abbildungen: true;
    };
...
try {
    ...
    const filme: FilmMitTitelUndAbbildungen[] = await prisma.film.findMany({
        // shorthand property
        where,
        include: includeTitelAbbildungen,
    });
    printResult('Filme mit Abbildungen (Titel enthält "n")', filme);

    // union type
    const titel = filme.map((b) => b.titel?.titel);
